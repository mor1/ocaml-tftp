...
module Make(FS:V1_LWT.KV_RO)(S:V1_LWT.STACKV4) : sig

  type t
  (** Server state record. *)

  val make: fs:FS.t -> s:S.t -> unit -> t
  (** [make ~fs ~s] creates a server state record from
      a {! V1_LWT.KV_RO} containing the files to serve, and
